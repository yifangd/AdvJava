...
public class ManyQueues {
    ...
    public static void main(String[] args) {
        ...
        ManyQueues sv = new ManyQueues(10);
        sv.run();

        executor.shutdown();
    }

    public ManyQueues(int size) {
        initBuf(size);
    }
    ...
    public Result goCommon(Callable<Result> producer,
                           Callable<Result> consumer,
...
//                atomicTail.set(currTail + 1);
        ...
        public long put() {
            ...
            while (!Thread.currentThread().isInterrupted() &&
                    atomicHead.get() <= startIndex) {
                Thread.yield();
            }
//            System.out.println("Producer head = " + atomicHead.get() +
//                    ", tail: " + atomicTail.get());
            buffer[currTail % bufSize] = prodCount++;
            atomicTail.set(currTail + 1);
            return prodCount;
        }
    }
...
//                atomicTail.lazySet(currTail + 1);
        ...
        public int put() {
            ...
            buffer[currTail & mask] = prodCount++;
            atomicTail.lazySet(currTail + 1);
