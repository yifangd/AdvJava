...
public class NettiQue {
	...
	private int mask;
	private StateObject[] buffer;

	public static void main(String[] args) {

		executor = Executors.newFixedThreadPool(3);

		NettiQue sv = new NettiQue(10);
		sv.run();

		executor.shutdown();
	}

	public NettiQue(int size) {
		initBuf(size);
	}
	...
	private void initBuf(int size) {
		...
		buffer = new StateObject[bufSize];
		mask = bufSize - 1;
	...
	public class ProducerQueueVolatile implements Callable<Result> {
		...
		public Result call() {
			...
			IntStream.range(0, reps).forEach(i -> {
				...
				buffer[currTail % bufSize] = new StateObject(i + "");
				tail = currTail + 1;
			});
	...
	public class ConsumerQueueAtomic implements Callable<Result> {
		...
		public Result call() {
			...
			IntStream.range(0, reps).forEach(i -> {
				...
				StateObject x = buffer[currHead % bufSize];
				finalResult ++;
	...
	public class ProducerQueueAtomicLazyMask implements Callable<Result> {
		...
		public Result call() {
			...
			IntStream.range(0, reps).forEach(i -> {
				...
				buffer[currTail & mask] = new StateObject(i + "");
				atomicTail.lazySet(currTail + 1);
			});
	...
	public class ConsumerQueueAtomicLazyMask implements Callable<Result> {
		...
		public Result call() {
			...
			IntStream.range(0, reps).forEach(i -> {
				...
				int currHead = atomicHead.get();
				StateObject x = buffer[currHead & mask];
				finalResult ++;
				atomicHead.lazySet(currHead + 1);
			});
			time = start.until(Instant.now(), ChronoUnit.MILLIS);
